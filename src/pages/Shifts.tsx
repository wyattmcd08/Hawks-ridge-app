...
import { shiftsInWeek, sumGross, computeWeeklyBreakdown } from '../utils/calculations'
...
function groupByWeek(shifts: Shift[]) {
  ...
  for (const s of sorted) {
    ...
    const label = `Week of ${sow.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`
      ...
      <div className="mt-6 space-y-6">
        {groups.map((g, gi) => (
          <div key={g.label}>
            <div className="mb-2.5 flex items-center justify-between px-1">
              ...
              <p className="text-sm font-semibold tabular-nums">{currency(sumGross(g.items))}</p>
            </div>
        ...
        {shifts.length === 0 && (
          <GlassCard className="p-10 text-center">
            ...
            <p className="mt-1 text-sm text-mute">Tap the + button to log your first shift.</p>
          </GlassCard>
        )}
      </div>
